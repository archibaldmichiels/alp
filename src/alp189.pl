...
[mapped(noun,[from:B1, to:C|FSnoun]),
 mapped(adj,[from:A, to:B|FSadj]),
 X is B1-1,
 (B1=B;B1=X),                   % only one intervening element 'sua quisquam sententia'
 constraint([number:Nb,gender:Gender,case:Case,class:Class,sem:Sem,lex:LexNoun],FSnoun),
...
msort(PLbignp, Sorted),
 contiguous(Sorted),      % BUT SEE quem omnes urbes expulsum a vobis ad se vocabunt Cic Pro Milone 38 104
 \+dup(Sorted),
...
[mapped(prpers,[from:A,to:B|FS1]),
 ...
 map(np,[pathlist:[p(A,B),p(B,X)],hp:[p(A,B)],index:_,distance:[0],         % the index will be instantiated when the parse is ready for output
           ...
           number:Nbres,person:3,gender:Gres,type:core,case:C,w:1,constituent_structure:pp3refl])].
...
[mapped(prpers,[from:A,to:B|FS1]),
 ...
 map(np,[pathlist:[p(A,B)],hp:[p(A,B)],index:_,distance:[0],         % the index will be instantiated when the parse is ready for output
           ...
           number:Nb,person:3,gender:G,type:core,case:C,w:1,constituent_structure:pp3refl])].
...
% Weight <3, % cannot be heavy if built as an adjunct
 ...
 map(adjunct,[pathlist:PL,distance:[0],class:adjunct,value:About,type:Type,w:Weight,constituent_structure:C_str])].
                                           ...
                                           pick(head:CHead,CSTR,SSTR1),
                                                     ifthenelse(  (member(lex:pp3refl,CHead) ,member(emphasis:no,CHead)),
                                                                                                ( pick(gender:GIN,CHead, CH1),
                                                                                                  ...
                                                                                                  append([constituent_structure:NSSTR],PP1,PCNew),
...
% same should be done for object_i (special indirect object) and agent and more !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 map(pred,[cat:pred,type:finite,
             ...
             constituent_structure:[Lex,NewHead]])].
...
ifthen(   nonvar(PREPCPLT1),
                           constraint([index:IndexUP,lex:pp3refl,constituent_structure:[head:[gender:GUP,number:NUP,index:IndexUP]]],PREPCPLT1)),
...
% specific treament of se_binding because we have to enter into the constituent_structure to retrieve the head of the pp
...
ifthenelse(constraint([lex:Lex],Out),   % if the arg has a lexical head
           ...
           (
                                  constraint([constituent_structure:[head:[number:NumberinSpecs,gender:GenderinSpecs]]],Out),
                                                                         % retrieving gender and number from the proposed subject
                                  funify([number:Nsubj,gender:Gendersubj],[number:NumberinSpecs,gender:GenderinSpecs],[number:NNsubj,gender:GGsubj]),
                                  pick(constituent_structure:C_str,Out,NewOut),
                                 ...
                                 append([constituent_structure:C_strOut],NewOut,OutSpecs)
                                    ),
                                  OutSpecs=Out
                  ),
