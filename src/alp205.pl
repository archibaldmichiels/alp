...
% alp205 changes and additions marked by alp205 token
...
:- [wordforms,frames]. % wordforms is produced by makelex.pl
       % wordforms should be in the same dir as alp itself
       % idem for frames.pl, containing the verb, noun and adj frames
...
% a declaration that is taken care of in frames.pl
...
bag_words([autem,
           certe,
           denique,
           ecce, edepol,eheu,enim,enimuero,equidem,ergo,etiam,
           hercle,heu,
           igitur,itaque,item,
           mehercule,mehercle,
...
% The expansions are meant to cover grammatical structures that are not arguments (so-called 'satellites')
...
% lex(isti, adj, [pos:adj, txt:isti, lex:iste,gender:masc,type:tool,case:nom, number:pl]).
 ...
 [mapped(adj,[from:A,to:B|FS]),
  constraint([type:tool,lex:LexTOOL, gender:G,case:C,number:N],FS),
  mapped(adjp,ADJP),
  constraint([pathlist:PADJ,distance:[0],hp:HP,case:C,number:N,gender:G,
             lex:LexADJ,type:Type,w:1], ADJP),

 not(Type=tool),                                   % the second adj should be standard but iste+poss !!!! solution:  poss attached to nps ALP 201
 not(Type=poss),                                   % so that istam tuam amicitiam etc OK
...
% adjacent(PL1,PathlistArg),     % args before or after adj, but contiguous                             % REMMED !!!!!!!!!!!
...
% myplus(T1,1,W),
 ...
 % bonus weight for clause over np
  ...
  msort(FlatAll, NSorted),
  \+dup(NSorted),
  contiguous(NSorted),                                    % CONTIGUITY TEST IMPORTANT %%%%%%%%%%%% suspended for ALP Hor
                                  % quis gracilis etc
...
[mapped(noun,[from:A, to:B|FSnoun]),
 ...
 relaxadjacent([p(A,B)],[p(X,Y)]), % relaxed adjacency required: 'res(X)nouae' or 'nouae(X)res' 
 ...
 map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[0],cat:np,class:common,sem:[abstract],
          ...
          case:Case,w:10,constituent_structure:res_nouae])].
...
[mapped(noun,[from:A, to:B|FSnoun1]),
 ...
 constraint([number:sing,case:Case,lex:ciuitas],FSnoun1),        % urbs in the sing

 constraint([number:sing,gender:fem,case:Case,sem:[country],lex:Lex],FSnoun2),     % any civitas
                                                                                % we can use the sem feature
                    % remember that the value of the sem feature
                    % should always be a list

 map(np,[pathlist:[p(A,B),p(B,C)],hp:[p(A,B)],index:i(p(A,B)),distance:[0],cat:np,class:common,sem:[country],
          number:sing,person:3,gender:fem,type:core,lex:Lex,lextype:full,
          case:Case,w:2,constituent_structure:Lex])].
...
% ALP 201  rewritten with np node instead of just noun
...
[
  mapped(np,FSnp),
  constraint([pathlist:Pnp1,hp:HP],FSnp), 

  mapped(adj,[from:A, to:B|FSadj]),
  start(Pnp1,StartNP),
 B =< StartNP,
  % the adj  precedes the noun phrase
  

  constraint([number:Nb,gender:Gender,case:Case,class:Class,sem:Sem,lex:LexNoun, index:Index,distance:Dis,w:Weight],FSnp),

  constraint([number:Nb,gender:Gender,case:Case,type:int,lex:_],FSadj),                % interrogative
  lex(LexNoun,noun,Features),     % the NP is built around a true noun

 append([p(A,B)],Pnp1, Ptot),
 msort(Ptot,Sorted),
\+dup(Sorted),
 

 map(np,[pathlist:Sorted,hp:HP,index:Index,distance:Dis,cat:np,class:Class,sem:Sem,
          number:Nb,person:3,gender:Gender,type:int,lex:LexNoun,lextype:full,
          case:Case,w:0,constituent_structure:[head:FSnp,adjp:FSadj]])].                 % ALP 201
...
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% np instead of n necessary
% in order to avoid competing nps in the next run
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
[mapped(np,FSnp),
 ...
 constraint([number:Nb,gender:Gender,case:Case,class:common,sem:Sem,lex:LexNoun, index:Index,distance:[DisNP],w:Weight],FSnp),
 constraint([number:Nb,gender:Gender,case:Case,type:poss,lex:_],FSadj),                % possessive

 lex(LexNoun,noun,Features),     % the NP is built around a true noun

 distance(HP,[p(A,B)],Dis),

 Dis < 4,
 % only a few intervening elements 'sua quisquam sententia'  etc.

 append([p(A,B)],Pnp1, Ptot),
 msort(Ptot,Sorted),
\+dup(Sorted),
  myplus(DisNP,Dis,DisTot),




 map(np,[pathlist:Sorted,hp:HP,index:Index,distance:[DisTot],cat:np,class:common,sem:Sem,
          number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
          case:Case,w:Weight,constituent_structure:[head:FSnp,adjp:FSadj]])].

% Class is specified as common to prevent possessive adj from getting attached to pronouns; note that personal pronouns are assigned to class 'proper'
% problematic for marcus meus etc...
...
[mapped(noun,[from:A, to:B|FSnoun]),
 ...
 constraint([number:Nb,gender:GenderAdj,case:Case2,lex:LexAdj,distance:DisAdj,type:Type,w:W],FSadj),  % distance ALP 199
...
% compatibility voc and nom
 ...
 Type \= int,         % interrogative and possessive adjectives treated elsewhere - see above 
 Type \= poss,
 ...
 ifthen(LexAdj=is, Distance=0),    % is/ea/id adjacent - this requirement is too strong (not with post-noun position, though)

 \+member(LexAdj,[tot,aliquot]),    % a lot of work necessary to complete list - adj that cannot follow noun
...
% we still have to exclude the occurrence, within the gap, of nouns to which the adjective could be
 ...
 ifthen(Distance=1, relaxedadjacent1_cgn([p(A,B)],Padj,CaseOut,Gender,Nb)),
  ...
  ifthenelse(Distance=0,myplus(W,2,Weight),myplus(W,1,Weight)),               % ALP 205 bonus for adjacency adj n 
  myplus(Distance,DisAdj,FDistance),                                          % ALP 199

  append([pathlist:[p(A,B)]],FSnoun,FSnounp),         % so that the noun should appear when the pathlists get pretty-printed
  map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[FDistance],      % distance is recorded as straining factor  FDistance ALP 199
...
% ALP 201 with np instead of noun REMMED !!!!!

[core,np8a] --->
[mapped(np,FSnp),
 mapped(adjp,FSadj),

 constraint([pathlist:Pnp1,hp:[p(A,B)]],FSnp),  % ALP 200 NP not bare Noun
 constraint([pathlist:Padj],FSadj),
 msort(Padj,Padjsorted),                          % ALP 194
 start(Padjsorted,Ext),
 Ext > B,                             % here the adj phrase follows the noun



 constraint([number:Nb,gender:Gender,case:Case1,class:common,sem:Sem,lex:LexNoun],FSnp), % class:common ALP 201

 constraint([number:Nb,gender:GenderAdj,case:Case2,lex:LexAdj,distance:DisAdj,type:Type,w:W],FSadj),  % distance ALP 199

 LexAdj \= hic,  % ALP 201 hic adj cannnot follow noun ?
...
%  append([pathlist:[p(A,B)]],FSnoun,FSnounp),         % so that the noun should appear when the pathlists get pretty-printed
  map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[FDistance],      % distance is recorded as straining factor  FDistance ALP 199
            cat:np,class:common,sem:Sem,                                         % class:common ALP 201
            number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
            case:CaseOut,w:Weight,
            constituent_structure:[head:FSnp,adjp:FSadj]])].
...
Type \= int,           % interrogative and possessive adjectives dealt with separately
Type \= poss,
...
% ifthen(Type=tool, Distance < 2),    % *haec* mihi si dederis *commoda* miles ero... hence the remming
 ...
 ifthen(Distance=1, relaxedadjacent1_cgn(Padj,[p(A,B)],CaseOut,Gender,Nb)),
  ...
  append([pathlist:[p(A,B)]],FSnoun,FSnounp), % so that the noun should appear when the pathlists get pretty-printed
  map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[FDistance],                                              % ALP 199
            cat:np,class:Class,sem:Sem,
            number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
...
% ALP 200 adj attachment  carried out with respect to np instead of bare noun
% enables several adjectives to modify the noun, building a single np
...
[mapped(np,FSnp),
 ...
 constraint([number:Nb,gender:GenderAdj,case:Case2,lex:LexAdj,distance:DisAdj, type:Type,w:W1],FSadj), % distance ALP 199
...
%  append([pathlist:[p(A,B)]],FSnoun,FSnounp), % so that the noun should appear when the pathlists get pretty-printed
  map(np,[pathlist:Sorted,hp:[p(A,B)],index:i(p(A,B)),distance:[FDistance],                                              % ALP 199
            cat:np,class:common,sem:Sem,                                                           % class:common ALP 201
            number:Nb,person:3,gender:Gender,type:core,lex:LexNoun,lextype:full,
            case:CaseOut,w:Weight,constituent_structure:[head:FSnp,adjp:FSadj]])].
...
% ALP 200
ifthen(Type_pp=ppcentered,contiguous(Sorted)),
    % difficult and dangerous to do without : accensum desiderio reginae vertisse iter (accensum iter ??? !!!)
    % we can't apply the restriction to present participle clauses : torquatus volo parvulus e gremio matris suae porrigens manus...

    % suspended for Hor quis gracilis...

 \+dup(Sorted),
 % Weight is W+Weightnp+1,     % we try to give them a good weight by adding the little bonus of 1
 myplus(W,Weightnp,T1),
 myplus(T1,1,Weight),
 map(np,[pathlist:Sorted,hp:HPnp,index:Index,distance:[GDD],   % GDD ALP 200
...
/*
lex(illa, adj, [pos:adj, txt:illa, lex:ille, gender:fem, type:tool, case:nom, number:sing]).

lex(illa, prdem, [pos:prdem, txt:illa, lex:prdemnomfemsing, gender:fem, person:3, sem:_, case:nom, number:sing, function:[subject]]).
*/

% [0/illa,1/mulier,2/lapidem,3/silicem,4/subigere,5/ut,6/se,7/amet,8/potest,endpos(9)]
...
[mapped(prdem,[from:A,to:B|FS1]),              % PRDEM : ditto for demonstrative pronouns
 constraint([number:Nb,gender:G, case:C,lex:Lex],FS1),
  ...
  % ifthenelse( (mapped(noun,[from:B,to:C|FSnoun]),constraint([number:Nb,gender:G,case:C],FSnoun)),Weight is 0, Weight is 1),
    % better read as an adj in that case

 map(np,[pathlist:[p(A,B)],hp:[p(A,B)],index:i(p(A,B)),distance:[0],
           cat:np,sem:_,class:dem,lex:Lex,            % class dem
           number:Nb,person:3,gender:G,type:core,case:C,w:0,constituent_structure:Lex])].  % ALP 201 simply downgrade pronoun reading
...
% ifthenelse( (mapped(noun,[from:B,to:C|FSnoun]),constraint([number:Nb,gender:G,case:C],FSnoun)),Weight is 0, Weight is 1),
 ...
 map(np,[pathlist:[p(A,B)],hp:[p(A,B)],index:i(p(A,B)),distance:[0],
           ...
           number:Nb,person:3,gender:G,type:core,case:C,w:0,constituent_structure:Lex])]. % ALP 201 simply downgrade etc
...
% should be confronted with code on dummy nps derivable from rel clause see lines 3612 and following or thereabouts
% ALP 201
...
append(PL1,PL2,PL),
 ...
 ifthenelse(contiguous(Sorted),Weight is TW+3,Weight is TW+1), % NOT an error - contiguousness is both a redeeming factor in case of distance
...
/*
% needs a change in the class of names of countries cities etc.
% not sure it's of much use

% second noun is a name; high weight 'civitas helvetia'
[core,np26a] --->
[mapped(noun,[from:A, to:B|FSnoun1]),
 mapped(noun,[from:B, to:C|FSnoun2]),
 ...
 constraint([class:proper, number:N,case:Case,gender:G,sem:Sem2,lex:Lex2],FSnoun2),
 ...
 map(np,[pathlist:[p(A,B),p(B,C)],hp:[p(A,B)],index:i(p(A,B)),distance:[0],cat:np,class:common,sem:Sem1,
          number:N,person:3,gender:G,type:core,lex:Lex1,lextype:full,
          case:Case,w:5, constituent_structure:[apposition_pair,head:Lex1,head:Lex2]])].
*/
...
%	d�ca				choisis � qui dire          : toi seule me plais
...
% TO BE REVISED
% see SUNT QUI above
% cannot do the same job twice

% ESSE in front : Existential (There is / There are
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[finite,np32b] --->
 ...
 [mapped(v,[from:0, to:1|FSverb]),
  ...
  constraint([pathlist:PL,hp:HP,distance:Dist,
             cat:np,type:full,class:common,
             index:Index,number:Nb,gender:G,sem:SemNP,person:3,case:Case_NP,lex:Lex,w:W], NP),
  


 append([p(0,1)],PL,PLtot),
 ...
 map(pred,[cat:pred,
              ...
              flagint:_,
              constituent_structure:[illocutionary_force:existential,vg:FSverb|NP]])].
...
% Do we need the IN CORE run?  It does not seem to do any harm and is not responsible for parser inefficiency
% It also seems necessary. The following is evidence: Grandia per multos tenuantur flumina rivos.
...
% ab initio, a principio, ab hora tertia

[core,adjunct3] --->
[mapped(pp,FSpp),
 constraint([pathlist:PL,distance:Dist,
           case:abl,prep:ab,lex:or([principium,initium,hora]),
...
% Weight <3, % cannot be heavy if built as an adjunct
 ...
 map(adjunct,[pathlist:PL,distance:Dist,class:adjunct,value:About,type:Type,w:Weight,constituent_structure:C_str])]. % 0.5 ALP 197



% ALP 205 : w:Weight   up to ALP204 incl : w:0.5
...
[ mapped(v,[from:X, to:Y|FSverb]),
  ...
  mapped(p_p,[from:C, to:D|FSpp]),
  adjacent([p(X,Y)],[p(C,D)]),               % adjacency sum/pp  necessary for bonus as passive : w:20
                                             % if not adjacent, interpretation as esse + participial clause is preferred
                                             % gallia est omnis divisa in partes tres : sum + pp-clause
					     % gallia est divisa in partes tres : adjacent -> passive for ALP!	????????

                                             % but we do find 'nam omnis civitas Helvetia in quattuor pagos divisa est' BG 1 12 4
                                             % which is NOT a true passive

                                             % should we reduce bonus to 5?
...
% without esse: passive voice : asia in tres partes divisa.
...
% GERUND with ESSE understood
% lex(capiendum, v, [pos:v, class:tr_cod, type:gerund, lex:capere, txt:capiendum, kind:std, mood:gerund, person:3, case:acc]).
...
[ 
  ...
  constraint([case:acc,type:gerund,lex:Lex,kind:_],Gerund),          % accusative form of the gerund
  map(vgpos,[cat:vg,type:nonfinite,pathlist:[p(C,D)],hp:[p(C,D)],lex:Lex,
               person:3,mood:gerund,tense:present,
               voice:active,number:sing,gender:neuter,                  % middle voice
               value:obligation,w:0])].                                 % semantic force : obligation
 ...
 % we need a sophisticated pragmatic discourse-oriented module to deal adequately with this overgeneration
  ...
  insort(AllTrees, ST),                                          % standard order imposed on args IN THE PARSE TREE
   ...
   clause_constraints(Clause_Constraints,FSverb),   % they concern polarity (so far...)
            ...
            % prioritize normal subject-object order (following a suggestion made by Dominique Longr�e, ULg)
  ...
  map(pred,[cat:pred,type:nonfinite,           % type of the whole pred is nonfinite
              ...
              pathlistverb:PathlistVerb,          % ALP 201
...
% lex(uita, noun, [pos:noun, txt:uita, lex:uita, case:nom, gender:fem, class:common, ab:no, number:sing, sem:[thing, abstract, time]]).
...
[
   ...
   constraint([number:Nsubj,
              ...
              mood:Mood,
              tense:Tense,
              pathlist:PathlistVerb,   % we should percolate down that info if we wish to check that a subordinator precedes the 
                                       % main verb of its subordinate clause    ALP 201 comment
              lex:Clex,
              w:WVerb],FSverb),
...
ifthen( (constraint([object:[gender:GENDERinNP,number:NUMBERinNP]],ST),
  ...
  map(pred,[cat:pred,
              type:finite,
              pathlistverb:PathlistVerb,   % add ALP 201 if we wish to check that a subordinator precedes the main verb of the subclause
                                             % PathlistVerb is a true path, not necessarily a one word path; the 'precedes' relation should be used
...
[ mapped(sub,[from:A,to:B|Fsubordinator]),                 % we have a subordinator
 ...
 constraint([type:finite,class:m, pathlistverb:PathlistVerb,  
...
precedes([p(A,B)],PathlistVerb),                 % ALP 201
 ...
 map(pred,[cat:pred,type:finite,
             pathlist:Sorted,distance:Distance,pathlistverb:PathlistVerb,  % ALP 201
             gap:[],class:m,
...
[mapped(sub,[from:StartSubordinator,to:EndSubordinator|Fsubordinator]),        % subordinatOR
 ...
 precedes([p(StartSubordinator,EndSubordinator)],PathlistVerb),  % ALP 201
...
( extremity(Path1,X); extremity(Path1,Xminus1) ),
                                                    %        multas     res   et magnas                        (Cic, Pro Murena VIII 20)
                                                    % vide : constantis ducis et fortis !!!!  militis officium (Tac)


                     % the coordinator sits in between the 2 adj phrases -
                     % this condition amounts to an oversimplification

ifthenelse(extremity(Path1,Xminus1), MalusDis is 4, MalusDis is 0),              % ALP 199
...
( start(Path2,Y) ; start(Path2,Yplus)),
 ...
 myplus(Distance,MalusDis,FDistance),                % + MalusDis   ALP 199

 myplus(Weight1,Weight2,Weight),
 
 
 map(adjp,[cat:adjp,pathlist:Sorted,hp:HL,distance:[FDistance],             % ALP 199
...
[ mapped(coord,[from:X,to:Y|Fcoord]),
 ...
 constraint([type:finite,class:m,
             distance:[Distp1],
             w:W1,
             constituent_structure:[nominal_pred|Head1]],Fp1),
 constraint([type:finite,class:m,
             distance:[Distp2],
             w:W2,
             constituent_structure:[nominal_pred|Head2]],Fp2),
...
% Weight is W1+W2,
   myplus(Distp1,Distp2,Distance),
  myplus(W1,W2,Weight),
  myplus(Weight,1,TWeight), % bonus for conjoined preds
 ...
 map(pred,[cat:pred,type:finite,
             ...
             gap:[],
             w:TWeight,
             constituent_structure:[nominal_pred,head:Head1,coord:Coord, head:Head2]])].
...
% a distance of 1 or 2 (in the case of relaxadjacent2) is allowed between the two corresponding pairs
 ...
 relaxadjacent(PL1,PL2) :- adjacent(PL1,PL2),!.
...
ifthenelse(Psubj=3,W=(-3),W=0),     % the penalty applies only to third person subjects    % ALP 201 penalty to be changed
                                                                                           % according to text type and contents !!!!!


% ALP 201 PENALTY FOR CONTEXTUAL SUBJECT
% check : sua fata habent libelli    needs -3 to recognize libelli as subject
% ille sapientissimus vir ut Ithacam videret  needs -2 to keep ille sapientissimus vir together
% versabitur .... in campo exercitus : detach exercitus from campo : problem lies elsewhere, in the treatment of ellipsis
...
match(To_be_matched:Specs,To_be_matched:OutSpecs,[number:Nsubj,gender:Gendersubj,person:_],
                          PathlistArg,DistanceArg,ClauseType,gap:[],w:Wtout,Int,PathlistVerb) :-
...
binding_se(ClauseType,Nsubj,Gendersubj,Out,OutSpecs,Bonus),
myplus(W,1,Wtot),
...
binding_se(finite,_,_,Out,OutSpecs,Bonus):-
...
ifthenelse(mapped(flags,active(se)),  % IF1 CONDITION
                                   ...
                                   ( ifthenelse(member(object:ObjectIn,CS),   % IF 2 CONDITION  IF 1 THEN ACTION
                                                     ...
                                                     pick(object:ObjectIn,CS,H1), % IF 2 ACTION
                                                             ifthenelse(  ( member(lex:pp3refl,ObjectIn) ,   % IF 3 CONDITION
                                                                          ...
                                                                          (Bonus is 0,OutSpecs=Out))),    % ELSE 3


                                         (Bonus is 0, OutSpecs=Out)); % ELSE 2

                                    ifthenelse(member(i_object:IObjectIn,CS),   % IF 2 CONDITION  IF 1 THEN ACTION


                                                       (
                                                     pick(i_object:IObjectIn,CS,H2), % IF 2 ACTION
                                                             ifthenelse(  ( member(lex:pp3refl,IObjectIn) ,   % IF 3 CONDITION
                                                                            member(emphasis:no,IObjectIn)
                                                                           ),
 
                                                                                    ( pick(gender:IGIN,IObjectIn, IOI1),   % THEN 3 ACTION
                                                                                      pick(number:ININ,IOI1,IOI2),
                                                                                      pick(index:IndexIIN,IOI2,IOI3),
                                                                                     append([gender:Gender,number:Number, index:Index],IOI3,IObjectOut),
                                                                                      append(H2,[i_object:IObjectOut],INewCS),
                                                                                      append(Out1,[constituent_structure:INewCS],OutSpecs),
                                                                                      Bonus is 1),
             ...
             (Bonus is 0, OutSpecs=Out)).  % ELSE 1
...
binding_se(ClauseType,_,_,Out,Out,0) :- not(ClauseType = finite).
...
ifthen((T=adjp;T=gerundive), constraint([number:Nsubj, case:or([nom,acc])],FSSpecs)),
               % Number constraint on predicatives seems jusfifiable; case is restricted to nominative and accusative

ifthen(T=np, constraint([case:or([nom,acc])],FSSpecs)),                                                                   % ALP 199
               % Number constraint on predicative NPs not jusfifiable; case is restricted to nominative and accusative
...
ifthen((T=advp,Int=int), precedes(PathlistArg,PathlistVerb)),  % ALP 202  ubi esses not esses ubi
