...
% ifthen(Type=tool, Distance < 2),    % *haec* mihi si dederis *commoda* miles ero...
 msort(Pnp, Sorted),
  \+dup(Sorted),
  Distance < 6 ,
...
[mapped(np,FSnp),
 ...
 distance(PLnp,PLPC,DisCl),
% DisCl < 3,                            % ad hoc cf. quem omnes urbes expulsum a vobis ad se vocabunt Cic Pro Milone 38 104
...
[mapped(np,Fnp1), % the head
 ...
 distancev(PL1,PL2,Strain),                  Strain < 2,   % distance between paths    one word max !!!
